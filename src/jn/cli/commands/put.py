...
@click.option("--plugin", help="Explicitly specify plugin to use (e.g., 'table', 'tabulate', 'csv', 'json')")
@click.option("--tablefmt", default="simple", help="Table format for table/tabulate plugins")
@pass_context
def put(ctx, output_file, plugin, tablefmt):
    ...
    try:
        ...
        write_destination(
            ...
            plugin_name=plugin,
            plugin_config={"tablefmt": tablefmt} if plugin in ("table", "table_", "tabulate", "tabulate_") else None
        )
