...
cat > .todo.jsonl << 'EOF'
{"id":"ct5k9m8ag0001test0001","text":"Buy groceries","status":"pending","priority":"medium","tags":[],"notes":[],"blockers":[],"parent":null,"due":null}
...
{"id":"ct5k9m8ag0003test0003","text":"Submit report","status":"done","priority":"medium","tags":["@work"],"notes":[],"blockers":[],"parent":null,"due":"2024-12-20"}
...
{"id":"ct5k9m8ag0005test0005","text":"Write tests","status":"pending","priority":"medium","tags":["@work"],"notes":[],"blockers":[],"parent":null,"due":null}
EOF
...
todo list 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "2. List pending only:" >> actual.txt
todo list pending 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "3. Filter by priority:" >> actual.txt
todo list high 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "4. Filter by tag:" >> actual.txt
todo list @review 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "5. Search by text:" >> actual.txt
todo search bug 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "6. Show statistics:" >> actual.txt
todo stats 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt

echo "7. Quick count:" >> actual.txt
todo count 2>/dev/null | strip_colors >> actual.txt
echo "" >> actual.txt
