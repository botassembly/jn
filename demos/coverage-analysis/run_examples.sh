...
#
# Demonstrates:
# - Using JN profiles for coverage.json analysis
...
echo ""
echo "This demo uses 7 JQ profiles to analyze pytest coverage.json files."
echo "All profiles work on ANY coverage.json file (no hardcoded paths)."
echo ""
...
$JN cat coverage.json | \
  $JN filter '@coverage/uncovered-functions' | \
...
$JN cat coverage.json | \
  $JN filter '@coverage/functions-below-threshold?threshold=60' | \
...
$JN cat coverage.json | \
  $JN filter '@coverage/files-by-coverage' | \
...
$JN cat coverage.json | \
  $JN filter '@coverage/poor-branch-coverage?threshold=70' | \
...
$JN cat coverage.json | \
  $JN filter '@coverage/largest-gaps?min_missing=3' | \
  $JN put gaps.json
...
$JN cat coverage.json | \
  $JN filter '@coverage/summary-by-module' | \
  $JN put summary.json
...
$JN cat coverage.json | \
  $JN filter '@coverage/hotspots?min_statements=10&max_coverage=70' | \
...
echo "  $JN cat coverage.json | $JN filter '@coverage/uncovered-functions'"
echo "  $JN cat coverage.json | $JN filter '@coverage/functions-below-threshold?threshold=80'"
echo "  $JN cat coverage.json | $JN filter '@coverage/summary-by-module'"
echo "  $JN cat coverage.json | $JN filter '@coverage/hotspots'"
echo ""
echo "All profiles work on ANY coverage.json file - try them on your own project!"
